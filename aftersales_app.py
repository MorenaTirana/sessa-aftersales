...
def run_sql(conn, q, p=None):
    cur = conn.cursor(); cur.execute(q, p or []); conn.commit(); return cur
...
def header():
    st.markdown('<div class="brand"><h1>SESSA AFTER SALES</h1><small>After Sales Dashboard</small></div>', unsafe_allow_html=True)
...
def require_auth() -> bool:
    ...
    try:
        if "DEMO" in st.secrets:  # opzionale
    ...
    with st.sidebar:
        ...
        if st.button("Entra", use_container_width=True):
            ok = auth_disabled or (user.strip() == USER and pwd == PASS)
            if ok:
                st.session_state.auth_ok  = True
        ...
        mode = "DISABILITATA" if auth_disabled else "ATTIVA"
        st.caption(f"🔒 Autenticazione: {mode} | bypass demo: aggiungi ?demo=1 alla URL")
    return False
    ...
    with st.sidebar:
        ...
        for page in PAGES.keys():
            ...
            if st.button(page, key=f"nav_{page}"):
                ...
                try: st.query_params.update({"nav": page})
                except Exception: st.experimental_set_query_params(nav=page)
                st.rerun()
